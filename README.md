...
For this step we are going to load a dynamic-tables skill into CoCo then get CoCo to create teh SQL required to create our dynamic table.
...
7. You should see a success message at the top of the screen once it has uploaded. 
...
1. Turn on **Plan Mode** (click the Plan toogle in CoCo to turn on). This allows CoCo to think through a multi-step task and design a plan on how the task can be achieved.  
...
> **What to look for:** CoCo will create the the SQL required to im plment the dynamici table. It should create a new file in your workspace. 
...
### Step 1.4 – Expore the dynamic-table skill
...
Type **/dynamic-table** into CoCo and select the dynamic-tables skill from the results list. This tells CoCo to use this particualr skill. Then copy and paste this prompt into CoCo:
...
2. Copy the SQL that CoCo creates, and paste it inot a new worksheet so you can reuse it later.

Now you have a Silver-grade AP invoices Dynamic Table that automatically updates from mutiple tables in the Bronze layer, an operating runbook generated by CoCo which leverages teh dynamic table skill, and a proof query you can rerun after each change to your dynamic table.

<!-- ------------------------ -->
## Demo 2: Managing changing requirements 

A month later, the business sends a set of new requirements that for the AP invoices pipeline. New source systems must be added, field requirements have changed, and some definitions need clarification. 
...
1. Upload each file to CoCo. CLick the **+** icon in CoCo and select **upload**. Find the first file you downaloded and upload it.
2. Repeat for the other 2 files. 
3. Copy the following promot into CoCo
...
1. Review the chnages made to the SQL file and choose to "Keep All"
2. Runt he revised SQL by selectign "Run All"

At this point you have built a dynamic data pipeline, and then updated it in response to changing business requirements.

<!-- ------------------------ -->
## Demo 3: Build an Agent on the Accounts Payabale data *(Optional)*
...
2. Click the "Keep All" button to retain the chnages.
3. Now ask Coco to deploy the semantic view and runa SQL query to validate the semantic view is working.
...
1. Copy the following prompt to CoCo:
    ...
    ```
2. If prompted click "Allow" to allow CoCo to save the agent specifiction and deploy the agent.

### Step 3.4 – Using skills to assess code against best practice.

Skills can include sub skills that can be used to help verify your code against best practices. For example the semantic-studio skill in CoCo has an "Audit" sub skill. Lets try it out.

1. Copy the following promot into CoCo
...
- Using Plan Mode to allow CoCo to think through mutli step assignemnts and produce a detailed plan prior to actually creating and running code.
- Utilsing bundled skills (`semantic-studio`) and uploading a custom skill (`dynamic-tables`)
